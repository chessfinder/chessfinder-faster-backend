...
import sttp.tapir.server.tests.tests.backendResource
import sttp.tapir.serverless.aws.lambda.zio.tests.all.* 
...
class AwsLambdaSamLocalHttpTest extends AnyFunSuite {
  ...
  testServer(in_path_path_out_string_endpoint.endpoint) { backend =>
  ...
  testServer(in_string_out_string_endpoint.endpoint) { backend =>
  ...
  testServer(in_json_out_json_endpoint.endpoint) { backend =>
    basicRequest
  ...
  testServer(in_headers_out_headers_endpoint.endpoint) { backend =>
    basicRequest
  ...
  testServer(in_input_stream_out_input_stream_endpoint.endpoint) { backend =>
    basicRequest.post(uri"$baseUri/api/echo/is").body("mango").send(backend).map(_.body shouldBe Right("mango"))
  }

  testServer(in_4query_out_4header_extended_endpoint.endpoint) { backend =>
    basicRequest
