...
//          assertZIO(stubVerification)(Assertion.isUnit)
//        },
        test("should be able to parse a lot of games") {

          val resource = uri"/pub/player/tigran-c-1137/games/2023/08"

          val stub = `chess.com`
            .expectsEndpoint("GET", "/pub/player/tigran-c-1137/games/2023/08")
            .returnsJson(
              JsonReader.readResource("samples/2023-08.json")
            )
            .stub()
          ...
          val stubVerification =
            `chess.com`.verify(1, "GET", "/pub/player/tigran-c-1137/games/2023/08")

          for
            games <- actualResult
            result <- assertTrue(games.games.nonEmpty)
          yield result
        }
      )
