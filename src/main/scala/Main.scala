...
object Main extends ZIOAppDefault:
  ...
  private val endpoints: List[ZServerEndpoint[GameFinder, Any]] = controller.rest ++ swaggerEndpoint ++ redocEndpoint
  ...
  def run = 
    (for {
      _ <- Server.serve(app).provide(Server.default ++ GameFinder.Impl.layer)
      _ <- ConsoleLive.printLine("HEEEEEEEEEEEEEEEEEEEEEEEEREEEEEEEEEEEE").orDie
    } yield ())
