...
func TestTagPairs(t *testing.T) {
	...
	g.AddTagPair("Draw Offer Declined", "Black")
	g.AddTagPair("Draw Offer Accepted", "Blue")
	g.AddTagPair("Draw Offer Recalled", "Red")

	g.RemoveAllTagPairs()
	tagPair = g.GetTagPair("Draw Offer Declined")
	if tagPair != nil {
		t.Fatalf("expected %s but got %s", "nil", "not nil")
	}
	tagPair = g.GetTagPair("Draw Offer Accepted")
	if tagPair != nil {
		t.Fatalf("expected %s but got %s", "nil", "not nil")
	}
	tagPair = g.GetTagPair("Draw Offer Recalled")
	if tagPair != nil {
		t.Fatalf("expected %s but got %s", "nil", "not nil")
	}
}
