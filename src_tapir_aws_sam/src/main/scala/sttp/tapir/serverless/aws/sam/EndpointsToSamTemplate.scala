...
private[sam] object EndpointsToSamTemplate {
  def apply(es: List[AnyEndpoint], options: AwsSamOptions): SamTemplate = {
    ...
    val roleArn = options.namePrefix + "LambdaRoleArn"
    ...
    val roleParameter = options.source match {
      case cs @ CodeSource(_, _, _, _, hasRole) if hasRole => Some(Parameter.Role)
      case _ => None
    }
    
    val params = roleParameter.map(roleParameter => Map(roleArn -> roleParameter))
      ...
      Resources = Map(
        functionName -> FunctionResource(
          options.source match {
            ...
            case cs @ CodeSource(_, _, _, environment, hasRole) =>
              FunctionCodeProperties(
                ...
                Role = if (hasRole) Some(s"!Ref $roleArn") else None
              )
          }
        ),
