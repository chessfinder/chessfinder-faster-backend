...
class VerifySamTemplateTest extends AnyFunSuite with Matchers {
  ...
  test("should match the expected yaml with code source and appropriate role") {
    val expectedYaml = load("code_source_template_with_role.yaml")

    val samOptions: AwsSamOptions = AwsSamOptions(
      "PetApi",
      source = CodeSource(runtime = "java11", codeUri = "/somewhere/pet-api.jar", "pet.api.Handler::handleRequest", hasRole = true),
      timeout = 10.seconds,
      memorySize = 1024
    )
